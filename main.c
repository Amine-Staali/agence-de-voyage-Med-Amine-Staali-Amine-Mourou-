...
{
    char Num[10];
    char lieu_depart[100];
    char lieu_arrive[100];
    char date_depart[11];
    char heure_emb[6];;
    int place_dispo;
    float prix;
...
voy* selection(voy *v)
{
    ...
    scanf("%s", v->lieu_depart);
    fflush(stdin);
    printf("Lieu d'arrive: ");
    scanf("%s", v->lieu_arrive);
    fflush(stdin);
    printf("Date de depart jj/mm/aaaa: ");
    scanf("%s", v->date_depart);
    fflush(stdin);
...
void aff_voy(voy* v)
{
    ...
    while (fread(v, sizeof(voy), 1, f) == 1)
    {
        if(!(strcmp(v->Num, ch)))
        {
            printf(" ----------------------------------------------------------------------\n");
            printf("| Heure d'embarquement: ");
            printf("%s", v->heure_emb);
            printf("\t\t\t\t%s\n",v->date_depart);
            printf("| Lieu de depart: ");
            printf("%s",v->lieu_depart);
            printf("\t\t\t\tLieu d'arrive: ");
            printf("%s\n",v->lieu_arrive);
            printf("| Ref: ");
            printf("%s\n",v->Num);
...
int existed_voy(voy* v)
{
    ...
    if(!(strcmp(date1,date2))){
        ...
        {
            res1 = cmpdate(v->date_depart,date1);
            res2 = cmpdate(v->date_depart,date2);
            if((res1 >= 0) &&  (res2 <= 0))
            {
                affiche_voy(v);
                bool = 1;
